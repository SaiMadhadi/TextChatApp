...
struct client* find_client_node_by_descriptor(int descriptor);struct client* find_client_node_by_ip(char ip[]);
...
void send_port(char* port) {
   ...
   strcat(message, "PORT ");
   strcat(message, port);
   send(server_descriptor_global, message, sizeof(message), 0);
}
